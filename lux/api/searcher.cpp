...
namespace Lux {
  ...
  : engine_(engine)
  {
    engine_.pimpl_->si->set_sys_cond(RES_W_POS);
  }
  ...
  {
    engine_.pimpl_->si->set_sys_cond(sys_cond);
  }
  ...
  ResultSet Searcher::search(const std::string &query, Condition &cond)
  {
    ...
    for (int i = rs.get_base(); i < rs.get_base() + rs.get_num(); ++i) {
      Document doc = engine_.pimpl_->ds->get(irs[i].doc_id);
  ...
  {
    return engine_.pimpl_->ds->get_raw(doc_id);
  }

  IndexResultSet Searcher::search_index(const std::string &query)
  {
    Query *q = QueryParser::parse(query);
    IndexResultSet irs = engine_.pimpl_->si->search(q);
    delete q;  
    return irs;
  }
